...
export function getMotionData (canvas, camera, breakImmediately=true, useBinaryColor=false) {
  ...
  const context = canvas.getContext('2d');
  const priorSaveData = context.getImageData(0, 0, canvas.width, canvas.height);
  ...
  for (var i = 0; i < rgba.length; i += 4) {
    ...
    if(priorImageData[camera.name] && priorImageData[camera.name].length) {
      const priorDataDiff = priorImageData[camera.name][i] * R_WAVELENGTH_CONTRIBUTION 
        + priorImageData[camera.name][i + 1] * G_WAVELENGTH_CONTRIBUTION 
        + priorImageData[camera.name][i + 2] * B_WAVELENGTH_CONTRIBUTION;
  ...
  // save this frame for next round
  priorImageData[camera.name] = priorSaveData.data;
